...
private final class TableViewPrefetchDataSourceNotSet
    ...
    func tableView(_ collectionView: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {}
...
open class RxTableViewDataSourcePrefetchingProxy
    ...
    /// Typed parent object.
    public weak private(set) var collectionView: UITableView?

    /// - parameter tableView: Parent object for delegate proxy.
    public init(collectionView: ParentObject) {
        self.collectionView = collectionView
        super.init(parentObject: collectionView, delegateProxy: RxTableViewDataSourcePrefetchingProxy.self)
    }

    // Register known implementations
    public static func registerKnownImplementations() {
        self.register { RxTableViewDataSourcePrefetchingProxy(collectionView: $0) }
    }
...
extension RxTableViewDataSourcePrefetchingProxy: UITableViewDataSourcePrefetching {
    ...
    public func tableView(_ collectionView: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {
        if let subject = _prefetchRowsPublishSubject {
            subject.on(.next(indexPaths))
        }

        (_requiredMethodsPrefetchDataSource ?? tableViewPrefetchDataSourceNotSet).tableView(collectionView, prefetchRowsAt: indexPaths)
    }
}
